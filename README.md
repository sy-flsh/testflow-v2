...
- c10-1: 사용자 **soft delete / 계정 탈퇴**(Company 단위 비활성화와 전역 탈퇴를 분리).
  ...
  - **MasterAdmin 복구 기반**: `restoreSoftDeletedUser(targetUserId, actorUserId)` service helper(`deletedAt/by/reason=null`, 세션 미자동생성, Company state/role/member 보존, `USER_RESTORED` best-effort). **권한 검사를 하지 않으므로 반드시 MasterAdmin guard 통과 후 호출**해야 하며, 이번 단계엔 이를 노출하는 public endpoint/UI 가 **없다**(향후 c10-2).
