...
- 기능명, 기능 설명, 화면 유형, 테스트 유형 기반 초안 생성
- 생성 결과는 바로 저장하지 않고 사용자 검토 후 선택 저장
- OpenAI API 또는 백엔드 AI API 실패 시 rule-based fallback
...
- Result 상태 저장: untested, passed, failed, blocked, skipped
- Runner 화면 또는 Drawer
- 결과 저장 후 다음 TC로 이동
...
9. 선택된 Test Case 기준으로 TestRunResult가 untested 상태로 생성된다.
...
- TestRunResult
- Defect
- DefectRelation 또는 defect_links
- ActivityEvent
- LocalBackupSnapshot
...
- Drawer 또는 side panel 형태를 사용할 수 있다.
- 제목, 설명, 사전조건, 우선순위, 상태, 태그, 기대 결과, 테스트 단계를 수정할 수 있어야 한다.

### Bulk Edit

- 여러 Test Case를 선택해 공통 필드를 일괄 변경한다.
- v0.1 대상 필드는 상태, 우선순위, 태그로 제한한다.
- 태그는 추가, 제거, 덮어쓰기 정책을 명확히 표시한다.
...
Test Run 생성 시 사용자는 현재 프로젝트의 Test Case를 선택한다. 저장 시 선택한 Test Case마다 초기 `untested` TestRunResult를 생성한다.
...
XLSX 템플릿 생성은 보안 리스크, 브라우저 번들 크기, 서버 처리 방식을 검토해 구현 시점에 결정한다. v0.1에서 직접 `.xlsx` 업로드는 지원하지 않는다.
...
| Defect와 Result 연결 누락 | 결함 추적 맥락이 끊길 수 있음 | Defect 생성 시 testCaseId/testRunResultId 필수 연결 |
