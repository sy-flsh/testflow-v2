...
import { isMasterAdminUserId } from "@/lib/auth/master-admin";
...
export async function GET() {
  try {
    ...
    // c10-2/c10-7: MasterAdmin nav 노출 판단용 boolean. session User.id binding 기준(email 아님).
    const isMasterAdmin = await isMasterAdminUserId(session.user.id);
